...
```{r}
Dat2 <- Dat %>%
  select(-c(Plot_Number, Average_CH_Seed_Mass))
```
...
Make Average CL seed mass into a binary response variable (mean and above = 1, below = 0)
```{r}
Dat3$Binary_Avg_Seed_Mass <- ifelse(Dat3$Average_CL_Seed_Mass > mean(Dat3$Average_CL_Seed_Mass), 1, 0)
```
...
Make the tree:
```{r}
SeedMassTree <- tree(Binary_Avg_Seed_Mass ~ ., data = TDat, method = "class")

plot(SeedMassTree) # hmmmmmm
text(SeedMassTree, cex = 0.7, adj = 0)
```
