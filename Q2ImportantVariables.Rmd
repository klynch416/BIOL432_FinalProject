...
```{r}
# Function to combine multiple ggplot objects into a single plot
multiplot <- function(..., cols = 1) {
  ...
  grid <- matrix(vector(mode = "list", length = n_rows * cols),
                 ncol = cols, nrow = n_rows)
  
  # Fill the plot grid with plots
  for (i in 1:length(plot_list)) {
    row_idx <- ceiling(i / cols)
    col_idx <- i %% cols
    if (col_idx == 0) col_idx <- cols
    grid[[row_idx, col_idx]] <- plot_list[[i]]
  }
  
  # Combine the plots into a single plot
  gridExtra::grid.arrange(grobs = grid, ncol = cols)
}
```
...
confusion_matrix_rf <- confusionMatrix(rf_predictions, test$binary_response)
